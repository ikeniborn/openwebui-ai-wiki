...
def test_owui_provision_calls_provisioner(tmp_path, monkeypatch):
    ...
    # Patch the module attribute: the CLI does a lazy `from owaw.owui.provision import
    # provision_agent` inside the command body, re-looked-up at call time after this patch.
...
def test_owui_provision_errors_without_openwebui(tmp_path, monkeypatch):
    monkeypatch.setenv("OWAW_DATA_DIR", str(tmp_path))
